  ...
  for i, m in enumerate(csv.DictReader(fp, delimiter='|'), 1):
    ...
    if i == 1:
      ...
      for k in m.keys():
        ...
        # look for lat/lon
        if not m[k]:
          properties[k] = {"type": "string", "store": "yes"}
        elif re.match(r'^(\-?\d+(\.\d+)?),\s*(\-?\d+(\.\d+)?)$', m[k].strip()):
          print "detected", k, "is a geo_point type"
          properties[k] = {"type": "geo_point", "store": "yes"}
        else:
          try:
            x = float(m[k])
            properties[k] = {"type": "float", "store": "yes"}
          except ValueError:
            if m[k].isdigit():
              properties[k] = {"type": "integer", "store": "yes"}
            #elif k == 'timestamp':
              #properties[k] = {"type": "date", "store" : "yes", "format": "dateOptionalTime"}
            else:
              properties[k] = {"type": "string", "store": "yes"}
